...
export const processNutritionLabelV2_OpenAI = async (
  ...
  const resizedBuffer = await sharp(nutritionLabelBuffer)
    ...
    .toBuffer();
  const resizedBuffer2 = await sharp(nutritionLabelBuffer)
    .resize(1024, 1024, {
      fit: "inside",
      withoutEnlargement: true,
    })
  ...
  const [nutritionServingsData, nutritionLabelData, nutritionLabelCategory] =
    await Promise.all([
      ...
      generateDataOpenAI<NutritionInfoDetails>(
        model,
        resizedBuffer2,
  ...
  if (nutritionServingsData && nutritionLabelData) {
    if (
      extractInstruction === "per serving" &&
      nutritionServingsData.servingSize
    ) {
